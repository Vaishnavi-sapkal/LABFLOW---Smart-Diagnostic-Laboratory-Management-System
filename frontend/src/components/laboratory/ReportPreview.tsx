    ...
    <article className="card overflow-hidden">
      <div className="border-b border-border bg-brand-50 p-5">
        <div className="flex items-start justify-between gap-4">
          <div><h2 className="text-lg font-semibold text-brand-700">LabFlow Diagnostic Report</h2><p className="text-xs text-ink-muted">{reportId}</p></div>
      ...
      <div className="p-5">
        <div className="grid gap-3 rounded-ui border border-border p-4 text-sm md:grid-cols-2">
          ...
          <div><span className="text-ink-muted">Sample ID</span><p className="font-semibold">LAB-2026-08421</p></div>
        ...
        <div className="mt-5 overflow-x-auto">
          <table className="min-w-full text-left text-sm">
            ...
            <tbody className="divide-y divide-border">
              {tests.map((test) => <tr key={test.parameter}><td className="py-3 font-medium">{test.parameter}</td><td className="py-3">{test.value}</td><td className="py-3 text-ink-muted">{test.unit}</td><td className="py-3 text-ink-muted">{test.range}</td><td className="py-3">{test.flag ?? 'Normal'}</td></tr>)}
            </tbody>
          </table>
        </div>
      </div>

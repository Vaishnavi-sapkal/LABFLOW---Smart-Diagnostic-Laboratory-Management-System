...
export class InternalServiceGuard extends JwtAuthGuard {
  ...
  canActivate(context: ExecutionContext) {
    ...
    const internal = this.metadata.getAllAndOverride<boolean>(INTERNAL_SERVICE_KEY, [context.getHandler(), context.getClass()]);
    const secret = this.config.get<string>('INTERNAL_SERVICE_SECRET');
    if (internal && secret && request.headers['x-internal-service-key'] === secret) {
